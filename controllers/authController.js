...
const createSendToken = (user, statusCode, res) => {
  ...
  // Remove password from the ouput
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 4) Check if user changed password affter the token was issued
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // 3) Check if user changed password affter the token was issued
      if (currentUser.changedPasswordAfter(decoded.iat)) {
        return next();
      }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  // 3) Update changedPasswordAt peroperty for the users
  // 4) log the user in
  createSendToken(user, 200, res);
});
